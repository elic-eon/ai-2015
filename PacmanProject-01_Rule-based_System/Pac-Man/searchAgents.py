...
class CleanerAgent(Agent):
    ...
    def getAction(self, state):
        ...
        food = state.getFood();
        if food[pos[0]+1][pos[1]] == True and Directions.EAST in actions:
            return Directions.EAST
        elif food[pos[0]-1][pos[1]] == True and Directions.WEST in actions:
            return Directions.WEST
        elif Directions.SOUTH in actions:
...
<<<<<<< HEAD
    ...
    def getAction(self, state):
        ...
        #if current == Directions.WEST or current == Directions.NORTH:
        ...
        while current not in legal:
            current = Directions.RIGHT[current]

        # apply action
        nextstate = state.generatePacmanSuccessor(current)
        for index in range(1, nextstate.getNumAgents()):
            ghost_dir = nextstate.getGhostState(index).getDirection()
            if ghost_dir not in nextstate.getLegalActions(index):
                continue
            nextstate = nextstate.generateSuccessor(index, ghost_dir)
        ghosts = nextstate.getGhostPositions();
        print(nextstate.getPacmanPosition())
        print(ghosts)
        if nextstate.getPacmanPosition() in ghosts:
            current = Directions.REVERSE[current]
        return current
